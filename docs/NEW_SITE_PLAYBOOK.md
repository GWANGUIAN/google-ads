...
- **Search-engine ownership verification — only Naver needs a build-time var for a new site.**
  - **Google Search Console**: no in-page meta tag needed — domain-level DNS verification through Cloudflare already covers it, since Cloudflare is the DNS host for every site's domain in this repo.
  - **Bing Webmaster Tools**: also no separate meta tag needed — use Bing Webmaster Tools' **"Import from Google Search Console"** option. Since GSC is already DNS-verified (above), importing from it verifies the site in Bing at the same time, with no HTML meta tag or file upload step. (This wasn't known when some existing sites were set up, so a couple of them — check before assuming — have a real `PUBLIC_BING_SITE_VERIFICATION` meta tag from the old manual-verification route; leave those as-is, no need to rip out a working verification, just don't set this up the manual way for new sites.)
  - **Naver Search Advisor**: the one exception — Naver has no DNS or GSC-import shortcut, so it genuinely requires the HTML meta tag route. Use `data/site.ts`'s `SITE_VERIFICATION.naver`, rendered by `BaseLayout.astro` as a `naver-site-verification` meta tag, sourced from the `PUBLIC_NAVER_SITE_VERIFICATION` build-time var (same non-secret Environment-Variable pattern as the AdSense vars in §8).
  - **Google AdSense**: verified via `public/ads.txt` (see §5/§8's `ads.txt` handling), not a meta tag — there is no AdSense-specific verification meta tag in this stack at all.
  - Net effect: a new site's `.env.example`, `env.d.ts`, `data/site.ts`, and `BaseLayout.astro` only need to carry `PUBLIC_NAVER_SITE_VERIFICATION` for this category — don't scaffold `PUBLIC_GOOGLE_SITE_VERIFICATION` or `PUBLIC_BING_SITE_VERIFICATION` into a new site by default. (`apps/pwcheckup` is the reference implementation of this trimmed-down pattern.)
