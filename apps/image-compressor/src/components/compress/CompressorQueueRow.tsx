...
import type { QueueItem } from "./CompressorWidget";
...
export default function CompressorQueueRow({
  item,
  onDownload,
  onRemove,
}: {
        ...
        <p className="text-xs text-neutral-500">
          ...
          {item.status === "done" && item.result && (
            <>
              ...
              {savings !== null && (
                <span className={savings > 0 ? "font-medium text-success-500" : "font-medium text-neutral-500"}>
                  ({savings > 0 ? "-" : "+"}
                  {Math.abs(savings)}%)
                </span>
              )}
              {!item.isLossy && <span className="ml-1 text-neutral-400">(lossless re-encode)</span>}
            </>
          )}
