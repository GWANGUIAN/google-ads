...
import type { QueueItem } from "./ResizerWidget";

export default function ResizerQueueRow({
  item,
  onDownload,
  onRemove,
}: {
  ...
  const nativeLabel =
    item.naturalWidth && item.naturalHeight ? `${item.naturalWidth}×${item.naturalHeight}` : "reading…";
  const targetLabel =
    item.targetWidth && item.targetHeight ? `${item.targetWidth}×${item.targetHeight}` : "…";
    ...
    <div className="flex items-center gap-4 border-b border-neutral-100 px-4 py-3 last:border-b-0">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-xs text-neutral-500">
          ...
          {item.clamped && (
            <span className="ml-1.5 text-accent-700">(already smaller than target, kept at original size)</span>
          )}
